...
#if defined(NDEBUG)
#error "EMIT cannot be compiled without assertions."
#endif
...
int64_t GetBlockValue(int nHeight)
{
    ...
    // Begin POW Phase
    if (nHeight == 0) {
	    // Developer Fund -- Block 1
	    nSubsidy = 445542 * COIN; // 445.542 Developer Fund
    } else if (nHeight <= 129599 && nHeight > 0) {
	    // PoW Phase 1 (Ending Block 129.600) - 90 Days
	    nSubsidy = 10 * COIN; // Emissions: 1.295.980, Cumulative: 1.741.522
    } else if (nHeight <= 259199 && nHeight >= 129600) {
	    // PoW Phase 2 (Ending Block 259.200) - 90 Days
	    nSubsidy = 7.50 * COIN; // Emissions: 972.000, Cumulative: 2.713.522
    } else if (nHeight <= 388799 && nHeight >= 259200) {
	    // PoW Phase 3 (Ending Block 388.800) - 90 Days
	    nSubsidy = 5.0 * COIN; // Emissions: 648.000, Cumulative: 3.361.522
    } else if (nHeight <= Params().LAST_POW_BLOCK() && nHeight >= 388800) {
	    // PoW Phase 4 (Ending Block 518.400) - 90 Days
	    nSubsidy = 2.5 * COIN; // Emissions: 324.000, Cumulative: 3.685.522

    // Begin POS Phase
    } else if (nHeight <= 1036799 && nHeight > Params().LAST_POW_BLOCK()) {
	    // PoS Phase 1 (Ending Block 1.036.800) - 360 Days
	    nSubsidy = 2.5 * COIN; // Emissions: 1.129.000, Cumulative: 4.981.522
    } else if (nHeight <= 1555199 && nHeight >= 1036800) {
	    // PoS Phase 2 (Ending Block 1.555.200) - 360 Days
	    nSubsidy = 1.25 * COIN; // Emissions: 648.000, Cumulative: 5.629.522
    } else if (nHeight <= 2073599 && nHeight >= 1555200) {
	    // PoS Phase 3 (Ending Block 2.073.600) - 360 Days
	    nSubsidy = 0.60 * COIN; // Emissions: 311.040, Cumulative: 5.940.562
    } else if (nHeight >= 2073600) {
	    // PoS Phase 4 (In perpetuity) 
	    nSubsidy = 0.10 * COIN; // Emissions: 525.600, Cumulative: 6.466.162 (at 14 years)
    }
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount)
{
	...
	if (nHeight <= 129599 && nHeight > 0) {
		// During PoW Phase 1; PoW: 5.0/MN: 5.0
		ret = blockValue / 2;
	} else if (nHeight <= 259199 && nHeight >= 129600) {
		// During PoW Phase 2; PoW: 3.375/MN: 4.125
		ret = blockValue / (100 / 55);
	} else if (nHeight <= 388799 && nHeight >= 259200) {
		// During PoW Phase 3; PoW: 2.0/MN: 3.0
		ret = blockValue / (100 / 60);
	} else if (nHeight <= Params().LAST_POW_BLOCK() && nHeight >= 388800) {
		// During PoW Phase 4: PoW: 0.875/MN: 1.625
		ret = blockValue / (100 / 65);
	} else if (nHeight > Params().LAST_POW_BLOCK()) {
		// PoS Phase 1: MN: 2.50
		// PoS Phase 2: MN: 1.25
		// PoS Phase 3: MN: 0.60
		// PoS Phase 4: MN: 0.10 (In Perpetuity)
		ret = blockValue;
	}
