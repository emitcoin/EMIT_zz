...
The EMIT project operates an open contributor model where anyone is
...
The following applies to code changes to the EMIT project, and is not to be
confused with overall EMIT Network Protocol consensus changes.

Whether a pull request is merged into EMIT rests with the project merge
...
Patches that change EMIT consensus rules are considerably more involved than
...
Where a patch set proposes to change the EMIT consensus, it must have been
...
The project leader is the release manager for each EMIT release.
